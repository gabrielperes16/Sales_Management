        ...
        <tbody>
            ...
            while ($user_data = mysqli_fetch_assoc($result)) {
                ...
                echo "<td>
                    ...
                    <a class='button' href='#' data-bs-toggle='modal' data-bs-target='#SomaModal' title='Soma'>
                        <svg xmlns='http://www.w3.org/2000/svg' width='37' height='45' fill='currentColor' class='bi bi-plus-square-fill' viewBox='0 0 16 16'>
...
<!-- Modal de Soma -->
<div class="modal fade" id="SomaModal" tabindex="-1" aria-labelledby="somaModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="somaModalLabel">Soma de todos os Valores</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p id="modal-pessoas"></p>
                <p id="modal-total"></p>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var modalElement = document.getElementById('SomaModal');
        var modal = new bootstrap.Modal(modalElement);
        
        function atualizarModal() {
            var buttons = document.querySelectorAll('[data-quantidade]');
            var totalQuantidade = 0;

            buttons.forEach(function(button) { // Adicionado o parêntese de fechamento aqui
                var quantidade = parseFloat(button.getAttribute('data-quantidade')) || 0;
                totalQuantidade += quantidade;
            });

            var resultado = totalQuantidade * 7.00;

            document.getElementById('modal-pessoas').innerHTML = 'A Total de Itens este Mês é : <strong>' + totalQuantidade.toFixed(2) + '</strong>.';
            document.getElementById('modal-total').innerHTML = 'A Soma de Todos os Valores é: <strong>R$ ' + resultado.toFixed(2).replace('.', ',') + '</strong>.';
        }

        modalElement.addEventListener('show.bs.modal', atualizarModal);

        var calculoModalElement = document.getElementById('calculoModal');
        var calculoModal = new bootstrap.Modal(calculoModalElement);
        
        calculoModalElement.addEventListener('show.bs.modal', function (event) {
